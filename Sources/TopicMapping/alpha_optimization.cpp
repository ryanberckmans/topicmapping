...
void word_corpus::optimize_alpha_sparse() {
    ...
    RANGE_loop(doc_number, docs_) {
        IT_loop(mapid, itm, gammas_ldav_map_.at(doc_number)) {
            //
            gammas_ldav_[doc_number][itm->first]=itm->second;
            //cout<<"doc_number "<<doc_number<<endl;
        }
        RANGE_loop(k, gammas_ldav_[doc_number]) {
            gammas_ldav_[doc_number][k]=max(gammas_ldav_[doc_number][k], alphas_ldav_[k]);
        }
    }
    optimize_alpha();
