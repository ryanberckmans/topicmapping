...
int distribution_from_cumulative(const DD & cum, DD & distr) {		
...
int cumulative_from_distribution(DD & cum, const DD & distr) {		
...
double compute_hypergeometric(int i, int k, int kout, int m) {
	...
	for(int h=0; h<int(den.size()); h++) if(den[h]<=0) {
		cerr<<"denominator has zero or less (in the hypergeometric)"<<endl;
		return 0;
	
	}
	
	for(int h=0; h<int(num.size()); h++) if(num[h]<=0) {
...
template<typename T>
T random_from_deque(deque<T> &sq)  {

	return sq[irand(sq.size()-1)];
